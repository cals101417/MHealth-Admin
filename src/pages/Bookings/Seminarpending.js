...
export function Seminarpending() {
  const [seminar, setSeminar] = useState([]);
  const SeminarCollection = collection(db, "book_seminar");
    ...
    if (searchValue) {
      result = filteredData.filter((u) => {
        return (
          ...
          u.seminarStatus.toLowerCase().includes(searchValue) ||
  ...
  }, [search, sort, asc, seminar]);

  const handleReset = () => {
    setSort("");
  };
  const updateStatus = async (id, seminarStatus) => {
    const seminarDoc = doc(db, "book_seminar", id);
    const newStatus = { seminarStatus: (seminarStatus = "Approved") };
    await updateDoc(seminarDoc, newStatus);
  };
  const cancelStatus = async (id, seminarStatus) => {
    const seminarDoc = doc(db, "book_seminar", id);
    const newStatus = { seminarStatus: (seminarStatus = "Cancelled") };
    await updateDoc(seminarDoc, newStatus);
  };
                            ...
                            >
                              <td className="px-6 py-3 ">
                                <div className="flex justify-start">
                                  <div>
                                    <img
                                      src={`data:image/jpeg;base64,${seminarBook.senderImage}`}
                                      className="avatar w-10 h-10 rounded-full border object-cover lg:mr-8"
                                    />
                                  </div>
                                  <div className="py-2 hidden lg:block">
                                    <span>
                                      {seminarBook.firstName},&nbsp;
                                      {seminarBook.middleName}&nbsp;
                                      {seminarBook.lastName}
                                    </span>
                                  </div>
                                </div>
                              </td>
                              <td className="px-4 py-3">{seminarBook.time}</td>
                              <td className="px-4 py-3">{seminarBook.date}</td>
                              <td className="px-4 py-3">
                                {seminarBook.address}
                              </td>
                              <td className="px-4 py-3">
                                {seminarBook.seminarStatus}
                              </td>
                              <td className="px-4 py-3">
